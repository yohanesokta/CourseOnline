...
const App = () => {
  ...
  return (
    <div className="p-3 xl:p-5  py-10 font-poppins">
      ...
      {(ViewAdd) ? <div className="md:max-w-md bg-white p-5 rounded-xl mt-5 flex flex-col gap-2 ">
      ...
      <div className="w-full rounded-2xl my-10 p-5  bg-white">
        ...
        <table className="w-full my-5 text-left text-sm xl:text-[14pt]">
          <thead>
            <tr >
              <th >name</th>
              <th>status</th>
          ...
          <tbody>
            {MentorData.map((element:any) =>
              <tr key={element.user_email}>
                <td className="max-w-28 pt-2 xl:pt-8 pr-2"><div className="flex w-full h-full "><img src={image} className="hidden xl:block h-10 mx-4 my-2 rounderd-full" alt="image" /><p className=" flex items-center">{element.username}</p></div></td>
                ...
                <td className="pt-2 text-center xl:pt-8"><button className="bg-blue-400 text-white rounded-2xl p-2"><BiPen /></button></td>
              </tr>
            )}
